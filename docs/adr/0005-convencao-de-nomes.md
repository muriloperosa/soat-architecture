...
- Pasta namespaceada por agregado usa nome de arquivo genérico (`model.go`, `repository.go`, `handler.go`, `dto.go`, `mapper.go`, `routes.go`) — dentro da pasta do agregado o pacote já desambigua, não precisa prefixo. `internal/infrastructure/http/` segue essa regra: cada domínio ganha pasta própria (`http/auth/`, `http/health/`, `http/<dominio>/`), com nomes sem prefixo (`interno_handler.go`/`cliente_handler.go` continuam distintos por papel, não por domínio, já que os dois vivem dentro de `http/auth/`). `http/httperror/` é a única pasta que não é domínio: concentra `ErrorResponse`/`Respond*Error`, usados por todo handler HTTP — fica fora da raiz e de qualquer domínio de propósito, porque é pacote-folha (só importa `domain/shared`) e isso evita ciclo de import entre `router.go` (raiz, importa cada domínio pra registrar rota) e os domínios (que precisam responder erro). Só arquivos que não pertencem a nenhum domínio específico, como `router.go`, ficam soltos na raiz de `http/`.
