...
type OrcamentoResumoResponse struct {
	ID                uint64  `json:"id" example:"1"`
	ValorItemServicos float64 `json:"valor_item_servicos" example:"300"`
	ValorItemPecas    float64 `json:"valor_item_pecas" example:"150"`
	ValorTotal        float64 `json:"valor_total" example:"450"`
	Observacoes       string  `json:"observacoes" example:"Orçamento inicial"`
}
...
type OrdemServicoResumoResponse struct {
	...
	DataCadastro         time.Time                `json:"data_cadastro"`
	DataAtualizacao      time.Time                `json:"data_atualizacao"`
	Orcamento            *OrcamentoResumoResponse `json:"orcamento,omitempty"`
}
