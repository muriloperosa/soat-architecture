...
func TestNewLiberarReservaPecaUseCase(t *testing.T) {
	...
	useCase := NewLiberarReservaPecaUseCase(reservaRepository)
...
func TestLiberarReservaPecaUseCaseExecutar_LiberacaoParcial(t *testing.T) {
	...
	reservaRepository.EXPECT().BuscarPorOrdemEPeca(ctx, uint64(10), uint64(1)).Return(r, nil).Once()
...
func TestLiberarReservaPecaUseCaseExecutar_ReservaNaoEncontrada_RetornaErro(t *testing.T) {
	...
	reservaRepository.EXPECT().BuscarPorOrdemEPeca(ctx, uint64(10), uint64(1)).Return(nil, domainreservapeca.ErrReservaNaoEncontrada).Once()
