...
func TestInativarUsuarioUseCase_Executar_InativaUsuarioAtivo(t *testing.T) {
	repo := mocks.NewUsuarioRepository(t)
	uc := appusuario.NewInativarUsuarioUseCase(repo)
	...
	repo.EXPECT().BuscarPorID(mock.Anything, uint64(1)).Return(existente, nil)
	repo.EXPECT().Atualizar(mock.Anything, mock.AnythingOfType("*usuario.Usuario")).
		Run(func(ctx context.Context, u *domainusuario.Usuario) { require.False(t, u.Ativo()) }).
		Return(nil)

	require.NoError(t, uc.Executar(context.Background(), 1))
}

func TestInativarUsuarioUseCase_Executar_UsuarioNaoExiste_RetornaNotFound(t *testing.T) {
	repo := mocks.NewUsuarioRepository(t)
	uc := appusuario.NewInativarUsuarioUseCase(repo)
...
func TestInativarUsuarioUseCase_Executar_ErroDoBancoAoBuscar_RetornaInternalError(t *testing.T) {
	repo := mocks.NewUsuarioRepository(t)
	uc := appusuario.NewInativarUsuarioUseCase(repo)
...
func TestInativarUsuarioUseCase_Executar_ErroDoBancoAoAtualizar_RetornaInternalError(t *testing.T) {
	repo := mocks.NewUsuarioRepository(t)
	uc := appusuario.NewInativarUsuarioUseCase(repo)
