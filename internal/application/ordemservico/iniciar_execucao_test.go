...
func TestIniciarExecucaoUseCase_ComSucesso(t *testing.T) {
	...
	repository.EXPECT().Atualizar(mock.Anything, os).
		Run(func(_ context.Context, atualizada *domain.OrdemServico) {
			require.Equal(t, domain.StatusEmExecucao, atualizada.Status())
	...
	output, err := uc.Executar(context.Background(), app.IniciarExecucaoInput{
		OrdemServicoID: 42,
		UsuarioID:      7,
	})
	...
	require.Equal(t, domain.StatusEmExecucao.String(), output.Status)
}

func TestIniciarExecucaoUseCase_OSInexistente(t *testing.T) {
	repository := ordemservicomocks.NewOrdemServicoRepository(t)
	repository.EXPECT().BuscarPorID(mock.Anything, uint64(999)).Return(nil, domain.ErrOrdemServicoNaoEncontrada)

	uc := app.NewIniciarExecucaoUseCase(repository)
	_, err := uc.Executar(context.Background(), app.IniciarExecucaoInput{OrdemServicoID: 999, UsuarioID: 7})
...
func TestIniciarExecucaoUseCase_ErroAoPersistir(t *testing.T) {
	repository := ordemservicomocks.NewOrdemServicoRepository(t)
	os := ordemServicoAprovada(t)
...
func ordemServicoAprovada(t *testing.T) *domain.OrdemServico {
	t.Helper()
	...
	historico := []domain.HistoricoStatus{
		...
		domain.ReidratarHistoricoStatus(2, 42, domain.StatusAprovada, atualizacao, 3, ""),
	}
